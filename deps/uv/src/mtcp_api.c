...
int node_mtcp_init_wrapper() 
{
	...
	ret = mtcp_init("/home/stack/asim/uttam/node-asim/mtcp.conf");
	if (ret) {
...
int setting_up_mtcp_context() {
	...
	//if ((unsigned)rte_socket_id() == sock_id || sock_id == (unsigned)-1 || 1) {
	...
	mctx = mtcp_create_context(mtcp_context_coreid);
	if (mctx == NULL) {
